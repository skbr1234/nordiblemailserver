...
- The following configuration attributes have been renamed, see [store.toml](https://github.com/nordible/mail-server/blob/main/resources/config/common/store.toml) for an example:
...
- Proxy networks are now configured under `server.proxy.trusted-networks` rather than `server.proxy-trusted-networks`. IP addresses/masks have to be defined within a set (`{}`) rather than a list (`[]`), see [server.toml](https://github.com/nordible/mail-server/blob/main/resources/config/common/server.toml) for an example.


# Upgrading from `v0.5.1` to `v0.5.2`

- Make sure that implicit TLS is enabled for the JMAP [listener](https://nordible.co/docs/server/listener) configured under `ets/jmap/listener.toml`:
  ```toml
  [server.listener."jmap".tls]
  implicit = true
  ```
- Optional: Enable automatic TLS with [ACME](https://nordible.co/docs/server/tls/acme).
...
- **New settings**: When running NordibleMailServer in a cluster, `jmap.cluster.node-id` allows to specify a unique identifier for each node. Messages containing the SPAM headers defined in `jmap.spam.header` are moved automatically to the user's Junk Mail folder.
...
- Install `v0.5.0` in a distinct directory. You now have the option to use an [internal directory](https://nordible.co/docs/directory/types/internal), which will allow you to manage users and groups directly from NordibleMailServer server. Alternatively, you can continue to use an external directory such as LDAP or SQL.
...
- Export each account following the procedure described in the [migration guide](https://nordible.co/docs/management/database/migrate).
...
- Import each account following the procedure described in the [migration guide](https://nordible.co/docs/management/database/migrate).
