...
- **A brand-new CLI** (`nordiblemailserver-cli`) that also uses the JMAP API and can be used for day-to-day administration, scripted deployments, and infrastructure-as-code workflows. Full documentation is available at https://stalw.art/docs/management/cli.
...
A full description of how `v0.16` composes the published URLs, how the proxy can talk to NordibleMailServer on either HTTP or HTTPS, and where to set `NORDIBLEMAILSERVER_HTTPS_PORT` for non-standard public ports lives at https://stalw.art/docs/server/reverse-proxy/overview.
