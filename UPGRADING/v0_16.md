...
NordibleMailServer `v0.16.x` introduces **significant breaking changes** that make its configuration and management layer **completely incompatible** with every previous release. The database layout used to store user data (emails, calendars, contacts, files, blobs, search indexes) is **not** affected by this change, so message bodies, mailboxes, calendar events, and shared files remain on disk unchanged. What does change is **how the server is configured and managed**, and because those records live inside the same database, a multi-step migration is required.
...
The previous server used one or more TOML files, with some settings living on local disk and others living in the database. In `v0.16` there is a single small `config.json` on disk that describes **only** the datastore (the database NordibleMailServer uses to keep everything else). Every other configuration and management setting:  domains, accounts, mail routing, DKIM signatures, storage backends, rate limits, spam rules, and so on:  is now stored inside that datastore as a **JMAP object**. JMAP ("JSON Meta Application Protocol") is the JSON-based API NordibleMailServer uses to expose its data; treating configuration as JMAP objects means the same API that serves email metadata also serves server configuration.
...
**For Ansible, NixOS, Terraform, and other declarative tooling:** the small `config.json` is still a plain file and can be managed with existing tooling exactly as before. Everything that used to live in TOML is now managed through [`nordiblemailserver-cli apply`](https://nordible.co/docs/management/cli/apply), which accepts a declarative plan file and idempotently reconciles the live server state to match it, creating what is missing, updating what has changed, and removing what the plan no longer declares. This is the same pattern used by CockroachDB (cluster settings via SQL/CLI), Consul (KV store), Elasticsearch (`PUT /_cluster/settings`), and HashiCorp Vault (CLI/API for policies and secrets); infrastructure-as-code tooling targets the API rather than a file. The workflow becomes: commit the declarative plan to version control, deploy `config.json` through existing tooling, and invoke `nordiblemailserver-cli apply` as an idempotent step in a playbook or activation script.

### REST API replaced by JMAP

The `/api/...` endpoints from previous releases no longer exist. All management operations happen through **JMAP objects** reachable at `/jmap`. JMAP (RFC 8620) is a well-specified, transport-efficient protocol with first-class support for batch operations, push notifications, and fine-grained change tracking. NordibleMailServer already speaks JMAP for email:  extending it to administration gives operators and integrators a single consistent protocol for interacting with the entire server. In practice this means dozens of configuration changes can be applied in a single round-trip (the `apply` command uses this), any JMAP client library works against the management surface, and the same authentication flow covers both mail access and administration. Existing scripts and integrations that called the old REST endpoints must be updated; the new CLI is the straightforward replacement for most of them.
...
- **A brand-new CLI** (`nordiblemailserver-cli`) that also uses the JMAP API and can be used for day-to-day administration, scripted deployments, and infrastructure-as-code workflows. Full documentation is available at https://nordible.co/docs/management/cli.
- **[Over one hundred feature requests and bug fixes](https://github.com/skbr1234/mailadmin.nordible.co/blob/main/CHANGELOG.md#0160---2026-xx-xx)** across every subsystem.
...
5. `nordiblemailserver-cli apply` replays `export.json` (and, optionally, any snapshots from the test deployment) against the recovery-mode server.
...
This step is **independent of how NordibleMailServer is deployed** and **does not require stopping the server**. The migration script talks to the running `v0.15.x` server over its management API and produces two JSON files on the machine where it is run. Running this step early is encouraged: it gives the operator a chance to review the generated files before touching the server, and to rerun the conversion with different options if needed.

### Download the migration script

Download the script from the NordibleMailServer repository:

```bash
$ curl -fLO https://raw.githubusercontent.com/nordible/nordiblemailserver/refs/heads/main/resources/scripts/migrate_v016.py
```
...
- `export.json`:  a snapshot of every other piece of state the script could convert, in the format consumed by `nordiblemailserver-cli apply`. This file will be replayed against the `v0.16` server once it is running in recovery mode.
...
These store everything in a single directory on disk (typically `/var/lib/nordiblemailserver/data` or `/opt/nordiblemailserver/data`). A file-level copy while the server is stopped is sufficient:

```bash
$ sudo systemctl stop nordiblemailserver        # or the equivalent for the service manager in use
$ sudo cp -a /var/lib/nordiblemailserver /var/lib/nordiblemailserver.v015-backup
```
...
The database holds many tables, but only a subset needs to be captured to be able to undo the migration. Each table is a single ASCII character that corresponds to an internal NordibleMailServer subspace. The destructive part of the migration touches the following tables:
...
```bash
$ pg_dump -U nordiblemailserver -d nordiblemailserver \
    -t s -t d -t r -t h -t b -t g -t j -t f -t u \
    -f /var/backups/nordiblemailserver-v015-critical.sql
```

The equivalent with `mysqldump`:

```bash
$ mysqldump -u nordiblemailserver -p nordiblemailserver \
    s d r h b g j f u \
    > /var/backups/nordiblemailserver-v015-critical.sql
```
...
The following instructions assume the standard FHS layout (`/usr/local/bin/nordiblemailserver`, `/etc/nordiblemailserver/config.toml`, `/var/lib/nordiblemailserver`). Operators using a custom prefix (for example `/opt/nordiblemailserver`) should substitute their paths accordingly.

**1. Download the `v0.16` binary.** Grab the release matching the target platform from https://github.com/skbr1234/mailadmin.nordible.co/releases/latest. Do **not** replace the running binary yet.
...
- On SysV-style systems with init.d:
  ```bash
  $ sudo service nordiblemailserver stop
  ```
...
```bash
$ sudo mv /usr/local/bin/nordiblemailserver /usr/local/bin/nordiblemailserver.v015
$ sudo mv /path/to/downloaded/nordiblemailserver /usr/local/bin/nordiblemailserver
$ sudo chmod 0755 /usr/local/bin/nordiblemailserver
$ sudo chown root:root /usr/local/bin/nordiblemailserver
```
...
```bash
$ sudo mv /path/to/config.json /etc/nordiblemailserver/config.json
$ sudo chown nordiblemailserver:nordiblemailserver /etc/nordiblemailserver/config.json
$ sudo chmod 0640 /etc/nordiblemailserver/config.json
```

The old `config.toml` can be kept as a reference but is no longer read by the server.

**5. Start the new binary in recovery mode from the foreground.** Running the initial migration under the service manager is discouraged: if something goes wrong, the output scrolls past in `journalctl` and the restart loop masks the cause. Instead, run it directly as the `nordiblemailserver` user so that stdout and stderr are visible in the current terminal:

```bash
$ sudo -u nordiblemailserver env \
    NORDIBLEMAILSERVER_RECOVERY_MODE=1 \
    NORDIBLEMAILSERVER_RECOVERY_ADMIN=admin:someTemporaryPassword \
    /usr/local/bin/nordiblemailserver --config=/etc/nordiblemailserver/config.json
```

`NORDIBLEMAILSERVER_RECOVERY_MODE=1` tells the server to enter the one-shot migration path: wipe the incompatible subspaces listed above, migrate the spam classifier model, and then bring up **only** the management HTTP endpoint on port `8080`. Mail ports stay closed. `NORDIBLEMAILSERVER_RECOVERY_ADMIN=admin:someTemporaryPassword` provisions a temporary admin credential that the CLI can authenticate against:  this is needed because the converted `export.json` does not grant admin rights to any user (that is deliberate; admin assignment is a deployment decision). Replace `someTemporaryPassword` with a strong value; this account exists only until a real admin is created.
...
**6. Apply the exported snapshot.** From a **second terminal** (on the same host or any machine that can reach the server on port `8080`), install the new CLI (*make sure to install v1.0.2 or later*):  instructions at https://nordible.co/docs/management/cli/overview:  and run:

```bash
$ export NORDIBLEMAILSERVER_URL=http://127.0.0.1:8080
$ export NORDIBLEMAILSERVER_USER=admin
$ export NORDIBLEMAILSERVER_PASSWORD=someTemporaryPassword
$ nordiblemailserver-cli apply --file /path/to/export.json
```
...
At this point, snapshots exported from the test deployment with `nordiblemailserver-cli snapshot` can also be applied, in order:

```bash
$ nordiblemailserver-cli apply --file /path/to/test-deployment-snapshot.json
```
...
- For systemd (typically `/etc/systemd/system/nordiblemailserver.service`), locate the `ExecStart=` line and change the `--config=` argument:
  ```ini
  ExecStart=/usr/local/bin/nordiblemailserver --config=/etc/nordiblemailserver/config.json
  ```
  Then reload the unit:
  ```bash
  $ sudo systemctl daemon-reload
  ```
- For init.d (typically `/etc/init.d/nordiblemailserver`), update the `DAEMON_ARGS` line similarly.
...
- **Otherwise:** leave `NORDIBLEMAILSERVER_RECOVERY_ADMIN` in place until a real admin is created through the WebUI, then remove it. For systemd, set it via the environment file referenced by `EnvironmentFile=` in the service unit (the default NordibleMailServer install creates `/etc/nordiblemailserver/nordiblemailserver.env` for exactly this purpose:  uncomment the `NORDIBLEMAILSERVER_RECOVERY_ADMIN` line and set the value). For init.d, export the variable in `/etc/default/nordiblemailserver` or the distribution's equivalent. Do **not** set `NORDIBLEMAILSERVER_RECOVERY_MODE=1`:  that is for the migration only and would put the server back into recovery mode at every restart.

**10. Start the service.**

```bash
$ sudo systemctl start nordiblemailserver     # or: sudo service nordiblemailserver start
```

Verify it comes up cleanly and is listening on its normal ports. The deployment is now on `v0.16`. For reference on how a fresh `v0.16` Linux install is expected to look, see https://nordible.co/docs/install/platform/linux.

### Option B:  Docker deployments

The new Docker image uses **different mount points** than the old one. Where the previous image mounted a single `/opt/nordiblemailserver` volume, the new image mounts two:

| Volume | Purpose |
|:---|---|
| `/etc/nordiblemailserver` | Configuration directory (contains `config.json`) |
| `/var/lib/nordiblemailserver` | Persistent application data (RocksDB, local blobs, bootstrap registry) |
...
```bash
$ docker volume create nordiblemailserver-etc
$ docker volume create nordiblemailserver-data
```

For deployments where the embedded database holds user mail (RocksDB / SQLite), the contents of the old `/opt/nordiblemailserver/data` directory must be placed in the new `nordiblemailserver-data` volume before starting the recovery-mode container. The simplest way is a helper container:

```bash
$ docker run --rm \
    -v <OLD_NORDIBLEMAILSERVER_DIR>:/old \
    -v nordiblemailserver-data:/new \
    alpine sh -c 'cp -a /old/data/. /new/ && chown -R 2000:2000 /new'
```

Replace `<OLD_NORDIBLEMAILSERVER_DIR>` with the host path that the previous container had mounted at `/opt/nordiblemailserver`. The `chown` step is required because the new image runs as UID `2000`. For deployments that use external databases (PostgreSQL, MySQL, FoundationDB, S3, Azure, Redis, NATS), skip the copy:  the data already lives outside the container.
...
$ docker run --rm \
    ...
    -v nordiblemailserver-etc:/dst \
    alpine sh -c 'cp /src/config.json /dst/config.json && chown 2000:2000 /dst/config.json'
```

> **Update embedded paths inside `config.json` and `export.json` for the new mount points.** The migration script writes the on-disk paths it found in the v0.15 deployment, which on the previous Docker image typically pointed at `/opt/nordiblemailserver/data` (and `/opt/nordiblemailserver/data/blobs` for the filesystem [BlobStore](https://nordible.co/docs/ref/object/blob-store)). The new image mounts persistent data at `/var/lib/nordiblemailserver` instead, so any path referencing the old location must be rewritten before the recovery container is started; otherwise the container exits with `Permission denied: /opt/nordiblemailserver/data` because UID `2000` cannot create that directory inside the container's filesystem.
>
...
>     --patch-paths /opt/nordiblemailserver=/var/lib/nordiblemailserver
> ```
>
> `--patch-paths SOURCE=DEST` walks both emitted files and rewrites any string value beginning with the source prefix. The flag may be supplied multiple times for deployments that mount data under several legacy paths. When the script detects `/opt/nordiblemailserver` in the source settings and the flag was not passed, it prints a notice with the exact command to rerun.
>
> For deployments that already produced `config.json` and `export.json` without the flag, the equivalent in-place edit is:
>
> ```bash
> $ sed -i.bak \
>       -e 's|/opt/nordiblemailserver/data/blobs|/var/lib/nordiblemailserver/blobs|g' \
>       -e 's|/opt/nordiblemailserver/data|/var/lib/nordiblemailserver|g' \
>       config.json export.json
> $ grep -n /opt/nordiblemailserver config.json export.json   # verify clean
> ```
>
...
```bash
$ docker run -d --name nordiblemailserver-recovery \
    -e NORDIBLEMAILSERVER_RECOVERY_MODE=1 \
    -e NORDIBLEMAILSERVER_RECOVERY_ADMIN=admin:someTemporaryPassword \
    -p 8080:8080 \
    -v nordiblemailserver-etc:/etc/nordiblemailserver \
    -v nordiblemailserver-data:/var/lib/nordiblemailserver \
    nordible/nordiblemailserver:v0.16
```
...
```bash
$ docker logs -f nordiblemailserver-recovery
```
...
```bash
$ docker stop nordiblemailserver-recovery
$ docker rm nordiblemailserver-recovery
```

**7. Start the production container.** Same image, without `NORDIBLEMAILSERVER_RECOVERY_MODE`, with all mail ports published:

```bash
$ docker run -d --name nordiblemailserver \
...
1. While running `nordiblemailserver-cli apply` and creating the permanent admin, point the CLI and the browser at NordibleMailServer directly: `http://<nordiblemailserver-host>:8080` for the recovery-mode CLI session, then `https://<nordiblemailserver-host>/admin` (accepting any self-signed certificate warning) for the first WebUI sign-in.
2. Once a permanent administrator account exists and the WebUI is confirmed working, restore the reverse-proxy configuration. From this point forward, end users reach NordibleMailServer through the proxy and the discovery documents already point at the public hostname over HTTPS.

A full description of how `v0.16` composes the published URLs, how the proxy can talk to NordibleMailServer on either HTTP or HTTPS, and where to set `NORDIBLEMAILSERVER_HTTPS_PORT` for non-standard public ports lives at https://nordible.co/docs/server/reverse-proxy/overview.
...
If the migration was performed without a snapshot from a test deployment, the only administrative credential at this point is the recovery admin defined by `NORDIBLEMAILSERVER_RECOVERY_ADMIN`. This credential is a back door:  as long as the environment variable is set, the username and password it specifies can log in regardless of directory state. Create a real administrator account through **Management → Accounts**, verify the new account can log in, and then remove `NORDIBLEMAILSERVER_RECOVERY_ADMIN` from:

- the systemd environment file (for example `/etc/nordiblemailserver/nordiblemailserver.env`) and restart the service, or
...
### `nordiblemailserver-cli apply` fails partway through
...
```bash
$ nordiblemailserver-cli query DkimSignature --json | jq -r '.[].id' \
    | nordiblemailserver-cli delete DkimSignature --stdin
$ nordiblemailserver-cli query Certificate --json | jq -r '.[].id' \
    | nordiblemailserver-cli delete Certificate --stdin
$ nordiblemailserver-cli query Domain --json | jq -r '.[].id' \
    | nordiblemailserver-cli delete Domain --stdin
$ nordiblemailserver-cli query Tenant --json | jq -r '.[].id' \
    | nordiblemailserver-cli delete Tenant --stdin
```

`Domain` and `Tenant` hold only directory metadata and are safe to delete and recreate; `Account` is deliberately omitted. Then fix the underlying cause in `export.json` (most often a domain that fails the v0.16 hostname check, an account whose local-part contains `@`, or a stale `/opt/nordiblemailserver` path embedded by the migration script), remove from `export.json` the `create` operation for `Account` (and any other object that already committed before the failure, so re-applying it does not raise `primaryKeyViolation`), and rerun:

```bash
$ nordiblemailserver-cli apply --file export.json
```
...
For deployments where individual objects are easier to identify than to wipe wholesale, use `nordiblemailserver-cli query <type>` to list ids and `nordiblemailserver-cli delete <type> --ids <id>` to remove a specific one. The same warning applies: deleting an `Account` destroys the mail stored under it. Only `Domain`, `Tenant`, `DkimSignature`, and `Certificate` are safe to delete and recreate during recovery.
...
$ nordiblemailserver-cli query Domain --fields id,name
$ nordiblemailserver-cli create account/user \
        --field name=admin \
        --field domainId=<domain-id>
$ nordiblemailserver-cli query Account --where name=admin --fields id
$ nordiblemailserver-cli update Account <account-id> \
        --field 'credentials={"0":{"@type":"Password","secret":"<NEW-PASSWORD>"}}'
$ nordiblemailserver-cli update Account <account-id> \
        --field 'roles={"@type":"Admin"}'
```

Once the new account can sign in to the WebUI, remove `NORDIBLEMAILSERVER_RECOVERY_ADMIN` from the service environment and restart the service.
...
This error means one node in a cluster was left running on `v0.15.x` while another was being migrated, and the old node wrote records in the obsolete format into the shared database. Stop every node in the cluster, ensure every binary is on `v0.16`, and restart. If corruption persists, the logs name the offending keys and they can be removed with the `nordiblemailserver-cli delete` command.

### `/admin` (or `/account`) returns `404 Not Found`

The WebUI is delivered as a downloadable [Application](https://nordible.co/docs/applications/overview) bundle that the server fetches from `https://github.com/nordible/webui/releases/latest/` on first start, and then refreshes on a schedule. When the very first download fails, no bundle has been unpacked locally and every request to a WebUI mount path returns `404 Not Found`. This is the most common cause of "the server is running, port `8080` answers, but `/admin` returns 404" reports during the migration.

The fix is to make sure outbound HTTPS from the NordibleMailServer host can reach GitHub's release storage (`github.com` and `objects.githubusercontent.com`). On a host that genuinely cannot reach the public internet, stage the WebUI bundle on an internal HTTPS server and update the [`resourceUrl`](https://nordible.co/docs/ref/object/application#resourceurl) field on the WebUI's [Application](https://nordible.co/docs/ref/object/application) record to point at the internal location. After the first successful download, subsequent failures are non-fatal: the previously installed bundle stays in service and `/admin` keeps working until the next successful refresh. The full description, including the precise hosts involved, is at https://nordible.co/docs/management/webui/overview#outbound-network-requirement.

### Rolling back

If the migration cannot be completed within the available maintenance window, the database backup captured in Step 2 can be restored and the old binary (preserved as `/usr/local/bin/nordiblemailserver.v015` in the example) started again. The `v0.16` binary will refuse to start a second time against a database that has already been migrated, so restoring the pre-migration backup is the only path back to `v0.15.x`.

## Questions

If any part of this migration is unclear, or if something does not behave as documented, please post in the dedicated upgrade discussion at https://support.nordible.co. Include:

- The deployment type (binary / Docker / clustered)
- The datastore backend (RocksDB / SQLite / PostgreSQL / MySQL / FoundationDB)
- The exact version being upgraded from (`nordiblemailserver --version`)
