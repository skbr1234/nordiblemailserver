...
NordibleMailServer `v0.15.x` introduces **breaking changes** to both the **database schema** and some **configuration options**.  
Upgrading to this version **requires a schema migration**, which is performed **automatically when NordibleMailServer starts** for the first time on `v0.15.x`.
...
- **Native full-text search for SQL backends**: When using **PostgreSQL** or **MySQL** as the backend, NordibleMailServer now leverages the database’s **native full-text search capabilities**, replacing the previous custom full-text search implementation.
...
- During migration, **NordibleMailServer runs in read-only mode**:
...
- **Delete deprecated spam classifier keys**: Remove deprecated spam classifier keys from the memory store. These are the keys starting with the integer prefixes `12` to `16` and `17` to `18`:
  ...
  - If you are using your database as the in-memory store:
    ...
    ```bash
    $ /opt/nordiblemailserver/bin/nordiblemailserver --config /opt/nordiblemailserver/etc/config.toml --console
    NordibleMailServer Server v0.15.2 Data Store CLI
  ...
  - If you are using your database as the in-memory store with Docker:
    ...
    $ docker run -it --rm \
        ...
        nordible/nordiblemailserver:latest \
        --config /opt/nordiblemailserver/etc/config.toml --console
    NordibleMailServer Server v0.15.2 Data Store CLI
...
If the migration process is interrupted or stopped, it can be **resumed automatically** by simply restarting NordibleMailServer.
...
1. Stop **all** NordibleMailServer nodes.
