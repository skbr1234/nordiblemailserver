#!/usr/bin/env python3
"""
NordibleMailServer v0.16 migration helper.

Two modes:

  dump    — pull all settings and principals from a NordibleMailServer server via the
            management API into two JSON files.

  convert — read those two JSON files and emit:
              * config.json   — plain DataStore object (NordibleMailServer's main config)
...
class NordibleMailServerClient:
    def __init__(
        self,
        base_url: str,
        *,
...
def cmd_dump(args: argparse.Namespace) -> int:
    if args.token:
        client = NordibleMailServerClient(args.url, token=args.token, verify=False)
    elif args.username and args.password:
        client = NordibleMailServerClient(
            args.url,
...
def detect_legacy_paths(settings: dict[str, str], prefix: str = "/opt/nordiblemailserver") -> int:
...
def cmd_convert(args: argparse.Namespace) -> int:
    ...
    if not patches and not args.keep_paths:
        legacy = detect_legacy_paths(settings, "/opt/nordiblemailserver")
        if legacy:
            print(
                f"notice: detected legacy Docker paths under /opt/nordiblemailserver in "
                f"{legacy} settings.\n"
                f"        the v0.16 Docker image mounts persistent data at "
                f"/var/lib/nordiblemailserver.\n"
                f"        rerun with --patch-paths /opt/nordiblemailserver=/var/lib/nordiblemailserver "
                f"to rewrite,\n"
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Dump / migrate a NordibleMailServer server via its management API.",
    )
    sub = p.add_subparsers(dest="command", required=True)

    d = sub.add_parser("dump", help="Dump settings and principals to JSON files.")
    d.add_argument("--url", required=True,
                   help="Base URL of the NordibleMailServer server, e.g. https://mail.example.com")
    ...
    c.add_argument("--patch-paths", action="append", type=parse_path_patch,
                   ...
                   help="Rewrite paths beginning with SOURCE to DEST in both "
                        ...
                        "/opt/nordiblemailserver to /var/lib/nordiblemailserver.")
