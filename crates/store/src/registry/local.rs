...
impl RegistryStoreInner {
    pub(crate) fn new(local_path: PathBuf) -> Self {
        let env_hostname = std::env::var("NORDIBLEMAILSERVER_HOSTNAME")
            .ok()
            .filter(|h| !h.is_empty())
            .unwrap_or_else(|| {
        ...
        Self {
            ...
            node_id: 0,
            env_recovery_mode: std::env::var("NORDIBLEMAILSERVER_RECOVERY_MODE")
                .ok()
                .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
                .unwrap_or(false),
            env_recovery_admin: std::env::var("NORDIBLEMAILSERVER_RECOVERY_ADMIN")
                .ok()
                .and_then(|v| {
                    v.split_once(':')
            ...
            env_cluster_role: std::env::var("NORDIBLEMAILSERVER_ROLE")
                .ok()
                .filter(|r| !r.is_empty()),
            env_push_shard_id: std::env::var("NORDIBLEMAILSERVER_PUSH_SHARD")
                .ok()
                .and_then(|id| id.parse::<u32>().ok().and_then(|v| v.checked_sub(1)))
                .unwrap_or(0),
            env_public_url: std::env::var("NORDIBLEMAILSERVER_PUBLIC_URL")
                .ok()
                .map(|v| v.trim().trim_end_matches('/').to_string())
                .filter(|u| !u.is_empty())
                .or_else(|| {
                    std::env::var("NORDIBLEMAILSERVER_HTTPS_PORT").ok().and_then(|p| {
                        p.parse::<u16>()
                            .ok()
