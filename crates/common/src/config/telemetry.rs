...
impl Tracers {
    pub async fn parse(bp: &mut Bootstrap, storage: &Storage) -> Self {
        ...
        if !bp.registry.is_recovery_mode() {
            ...
            let level = std::env::var("NORDIBLEMAILSERVER_RECOVERY_MODE_LOG_LEVEL")
                .ok()
...
impl Metrics {
    pub async fn parse(bp: &mut Bootstrap) -> Self {
        ...
        let resource = Resource::builder()
            .with_service_name("nordiblemailserver")
            .with_attribute(KeyValue::new(SERVICE_VERSION, env!("CARGO_PKG_VERSION")))
            .build();
        let instrumentation = InstrumentationScope::builder("nordiblemailserver")
