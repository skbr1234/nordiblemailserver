    ...
    async fn parse_http_request(
        ...
        match path.next().unwrap_or_default() {
            ...
            "api" => {
                ...
                // Allow CORS preflight requests from trusted loopback origins.
                if req.method() == Method::OPTIONS {
                    return Ok(
                        HttpResponse::new(StatusCode::NO_CONTENT)
                            .with_cors_localhost(cors_origin.as_deref()),
                    );
                }

                return match self.handle_api_request(&mut req, &session).await {
                    Ok(resp) => Ok(resp.with_cors_localhost(cors_origin.as_deref())),
                    Err(err) => {
                        let resp = (&err).into_http_response();
                        Ok(resp.with_cors_localhost(cors_origin.as_deref()))
                    }
                };
            }
            "mail" => {
            ...
            service_fn(|req: hyper::Request<body::Incoming>| {
                ...
                async move {
                    ...
                    if !server.core.network.http.response_headers.is_empty() {
                        let cors_headers_present = response
                            .headers()
