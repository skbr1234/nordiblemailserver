            ...
            if !deleted.is_empty() {
                ...
                if not_deleted.is_empty() {
                    self.write_ok(format!(
                        "Nordible POP3 bids you farewell ({num_deleted} messages deleted)."
                    ))
                    .await?;
                } else {
                    self.write_bytes(
                ...
                self.write_ok("Nordible POP3 bids you farewell (no messages deleted).")
                    .await?;
            }
        } else {
            self.write_ok("Nordible POP3 bids you farewell.").await?;
        }
