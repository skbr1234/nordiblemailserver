...
impl HttpResponse {
    ...
    pub fn with_cors_localhost(mut self, origin: Option<&str>) -> Self {
        let Some(origin) = origin.and_then(loopback_origin) else {
            return self;
        };
...
fn loopback_origin(origin: &str) -> Option<&str> {
    ...
    match host {
        "localhost" | "127.0.0.1" | "[::1]" => Some(origin),
        _ => None,
    }
}
