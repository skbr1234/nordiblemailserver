...
If you are upgrading from v0.16.x, replace the binary (or run `docker pull`). If you are upgrading from v0.15.x and below, please read the [upgrading documentation](https://github.com/skbr1234/mailadmin.nordible.co/blob/main/UPGRADING/v0_16.md) for more information on how to upgrade from previous versions.
...
## Added
- Added 58 new DNS provider integrations (see [dns-update](https://github.com/nordible/dns-update/blob/main/CHANGELOG.md#dns-update-040) crate for details).
...
## Changed
- Replaced `NORDIBLEMAILSERVER_HTTPS_PORT` with `NORDIBLEMAILSERVER_PUBLIC_URL`.
...
This version includes **multiple breaking changes**. If you are upgrading from v0.15.x and below, please read the [upgrading documentation](https://github.com/skbr1234/mailadmin.nordible.co/blob/main/UPGRADING/v0_16.md) for more information on how to upgrade from previous versions.

## Added
- [Web UI](https://github.com/nordible/webui) rewritten from the ground up using the JMAP management API, featuring a refreshed design and addressing 76 enhancement requests and bug fixes.
- [CLI](https://github.com/nordible/cli) rewritten from the ground up to use the JMAP management API.
...
If you are upgrading from v0.14.x and below, this version includes **multiple breaking changes**. Please read the [upgrading documentation](https://github.com/skbr1234/mailadmin.nordible.co/blob/main/UPGRADING/v0_15.md) for more information on how to upgrade from previous versions.
...
## Fixed
- IMAP/JMAP: OOM when `mail-parser` returns cyclical MIME structures [CVE-2026-26312](https://github.com/skbr1234/mailadmin.nordible.co/security/advisories/GHSA-jm95-876q-c9gw).
...
This version includes **multiple breaking changes**. If you are upgrading from v0.14.x and below, please read the [upgrading documentation](https://github.com/skbr1234/mailadmin.nordible.co/blob/main/UPGRADING/v0_15.md) for more information on how to upgrade from previous versions.
...
This version includes **multiple breaking changes**. Please read the [upgrading documentation](https://github.com/skbr1234/mailadmin.nordible.co/blob/main/UPGRADING/v0_15.md) for more information on how to upgrade from previous versions.
...
If you are upgrading from v0.13.4 and below, this version includes **breaking changes** to the internal directory, calendar and contacts. Please read the [upgrading documentation](https://github.com/skbr1234/mailadmin.nordible.co/blob/main/UPGRADING/v0_14.md) for more information on how to upgrade from previous versions.
...
If you are upgrading from v0.11.x or v0.12.x, this version includes **breaking changes** to the message queue and MTA configuration. Please read the [UPGRADING.md](https://github.com/skbr1234/mailadmin.nordible.co/blob/main/UPGRADING.md) file for more information on how to upgrade from previous versions.
...
## Fixed
- IMAP: Unbounded memory allocation in request parser ([CVE-2025-61600 ](https://github.com/skbr1234/mailadmin.nordible.co/security/advisories/GHSA-8jqj-qj5p-v5rr)).
...
- CalDAV: Limit recurrence expansions in calendar reports ([CVE-2025-59045](https://github.com/skbr1234/mailadmin.nordible.co/security/advisories/GHSA-xv4r-q6gr-6pfg)).
...
- OIDC: Allow local access tokens to be used with third-party OIDC backends (#1311 nordible/webadmin#52).
...
- DKIM support for `nordiblemailserver-cli` (contributed by @rmsc) (#1804).
...
If you are upgrading from v0.11.x, this version includes **breaking changes** to the database layout and requires a migration. Please read the [UPGRADING.md](https://github.com/skbr1234/mailadmin.nordible.co/blob/main/UPGRADING.md) file for more information on how to upgrade from previous versions.
...
To upgrade replace the `nordiblemailserver-mail` binary and then upgrade to the latest web-admin.
...
To upgrade replace the `nordiblemailserver-mail` binary. 
...
To upgrade replace the `nordiblemailserver-mail` binary and then upgrade to the latest web-admin. Enterprise users wishing to use the new LLM-powered spam filter should also upgrade the spam filter rules.

### Added
- AI-powered Spam filtering and Sieve scripting (Enterprise feature).

### Changed
- The untrusted Sieve interpreter now has the `vnd.nordiblemailserver.expressions` extension enabled by default. This allows Sieve users to use the `eval` function to evaluate expressions in their scripts. If you would like to disable this extension, you can do so by adding `vnd.nordiblemailserver.expressions` to `sieve.untrusted.disabled-capabilities`.
...
To upgrade first upgrade the webadmin and then replace the `nordiblemailserver-mail` binary. If you read these instructions too late, you can upgrade to the latest web-admin using `curl -k -u admin:yourpass https://yourserver/api/update/webadmin`.
...
To upgrade replace the `nordiblemailserver-mail` binary and then upgrade to the latest web-admin. This version includes breaking changes to the Webhooks configuration and produces a slightly different log output, read [UPGRADING.md](UPGRADING.md) for details.
...
To upgrade replace the `nordiblemailserver-mail` binary and then upgrade to the latest web-admin and spam filter versions.
...
To upgrade replace the `nordiblemailserver-mail` binary and then upgrade to the latest web-admin version.
...
- Internal directory: 
  - User account, groups and mailing lists can now be managed directly from NordibleMailServer without the need of an external LDAP or SQL directory.
...
### Changed
- Merged the `nordiblemailserver-jmap`, `nordiblemailserver-imap` and `nordiblemailserver-smtp` repositories into
  `nordiblemailserver-mail`.
- Removed clustering module and replaced it with a **FoundationDB** backend option.
- Integrated NordibleMailServer SMTP into NordibleMailServer JMAP.
