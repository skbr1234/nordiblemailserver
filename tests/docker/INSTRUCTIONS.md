# NordibleMailServer – Test Infrastructure

Ephemeral Docker Compose stack for testing NordibleMailServer against external services.
All data is lost on `docker compose down` – every restart is a clean slate.

## Quick Start

```bash
cd nordiblemailserver-test
docker compose up -d
```
...
|----------------|-------------------|-----------------|--------------------------------------------|
| PostgreSQL     | localhost         | 5432            | `nordiblemailserver` / `nordiblemailserver`, db: `nordiblemailserver`    |
| MySQL          | localhost         | 3306            | `nordiblemailserver` / `nordiblemailserver`, db: `nordiblemailserver`    |
| FoundationDB   | localhost         | 4500            | Cluster file from container                |
| Redis          | localhost         | 6379            | No auth                                    |
| OpenSearch     | localhost         | 9200            | No auth, security plugin disabled          |
| Meilisearch    | localhost         | 7700            | Master key: `nordiblemailserver-master-key`          |
| MinIO (S3)     | localhost         | 9000 / 9001     | `minioadmin` / `minioadmin`, bucket: `nordiblemailserver` |
| Keycloak (OIDC)| localhost         | 9080            | Admin: `admin` / `admin`                   |
| OpenLDAP       | localhost         | 389 / 636 (TLS) | Admin DN: `cn=admin,dc=nordiblemailserver,dc=test`, pw: `admin` |
...
| PowerDNS       | localhost         | 5300 / 8081     | API key: `nordiblemailserver-api-key`                |
| NATS           | localhost         | 4222 / 8222     | No auth                                    |

## OIDC (Keycloak) Details

- **OIDC Discovery**: `http://localhost:9080/realms/nordiblemailserver/.well-known/openid-configuration`
- **Token Endpoint**: `http://localhost:9080/realms/nordiblemailserver/protocol/openid-connect/token`
- **Client ID**: `nordiblemailserver`
- **Client Secret**: `nordiblemailserver-secret`
- **Realm**: `nordiblemailserver`
...
```bash
curl -X POST http://localhost:9080/realms/nordiblemailserver/protocol/openid-connect/token \
  -d "grant_type=password" \
  -d "client_id=nordiblemailserver" \
  -d "client_secret=nordiblemailserver-secret" \
...
- **Base DN**: `dc=nordiblemailserver,dc=test`
- **Admin DN**: `cn=admin,dc=nordiblemailserver,dc=test`
- **Admin Password**: `admin`
- **Read-only DN**: `cn=readonly,dc=nordiblemailserver,dc=test`
- **Read-only Password**: `readonly`
- **User DN pattern**: `uid={username},ou=users,dc=nordiblemailserver,dc=test`

### Test Users

| DN                                             | Mail                     | Password                 |
|------------------------------------------------|--------------------------|--------------------------|
| uid=john.doe,ou=users,dc=nordiblemailserver,dc=test      | john.doe@example.org     | this is an LDAP password |
| uid=jane.smith,ou=users,dc=nordiblemailserver,dc=test     | jane.smith@example.org   | this is an LDAP password |
| uid=bill.foobar,ou=users,dc=nordiblemailserver,dc=test    | bill.foobar@example.org  | this is an LDAP password |

### Groups

| DN                                          | Mail                    | Members          |
|---------------------------------------------|-------------------------|------------------|
| cn=sales,ou=groups,dc=nordiblemailserver,dc=test      | sales@example.org       | john.doe, jane.smith |
| cn=corporate,ou=groups,dc=nordiblemailserver,dc=test  | corporate@example.org   | bill.foobar, jane.smith |

### Example: Search by Email

```bash
ldapsearch -x -H ldap://localhost:389 \
  -D "cn=admin,dc=nordiblemailserver,dc=test" -w admin \
  -b "dc=nordiblemailserver,dc=test" "(mail=john.doe@example.org)"
```
...
- **API**: `http://localhost:8081` (API key: `nordiblemailserver-api-key`)
- **Zone**: `nordiblemailserver.test`
- **TSIG key name**: `nordiblemailserver-update-key`
...
```bash
dig @localhost -p 5300 _25._tcp.mail.nordiblemailserver.test TLSA
```

### Example: RFC2136 Dynamic Update

```bash
nsupdate -y hmac-sha256:nordiblemailserver-update-key:c3RhbHdhcnQtdGVzdC10c2lnLXNlY3JldC1rZXkxMjM0NTY3ODkw <<EOF
server 127.0.0.1 5300
zone nordiblemailserver.test
update add test.nordiblemailserver.test 300 A 192.168.1.100
send
EOF
```
...
- **TLS**: Self-signed — NordibleMailServer must trust the Pebble CA or skip TLS verification
...
Change them to match whatever port your test NordibleMailServer instance listens on.
...
curl -s -X POST http://localhost:8055/add-dns \
  -d '{"host": "_acme-challenge.mail.nordiblemailserver.test.", "value": "dns-challenge-token"}'

# Remove a DNS-01 TXT challenge response
curl -s -X POST http://localhost:8055/del-dns \
  -d '{"host": "_acme-challenge.mail.nordiblemailserver.test."}'
...
curl -s -X POST http://localhost:8055/add-tlsalpn \
  -d '{"host": "mail.nordiblemailserver.test", "content": "base64-encoded-key-authz"}'

# Remove a TLS-ALPN-01 challenge response
curl -s -X POST http://localhost:8055/del-tlsalpn \
  -d '{"host": "mail.nordiblemailserver.test"}'
...
- `localhost`, `keycloak`, `openldap`, `pebble`, `*.nordiblemailserver.test`, `127.0.0.1`

To extract the cert for use with NordibleMailServer:
