...
if [ ! -f "$CERT_DIR/cert.pem" ]; then
  ...
  openssl req -x509 -newkey rsa:2048 -nodes \
    ...
    -days 365 \
    -subj "/CN=localhost/O=NordibleMailServer Test/C=US" \
    -addext "subjectAltName=DNS:localhost,DNS:keycloak,DNS:openldap,DNS:pebble,DNS:*.nordiblemailserver.test,IP:127.0.0.1"
