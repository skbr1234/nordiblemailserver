    ...
    match typ {
        ...
        DataStoreType::PostgreSql => {
            ...
            DataStore::PostgreSql(PostgreSqlStore {
                host: "localhost".into(),
                port: 5432,
                auth_username: "nordiblemailserver".to_string().into(),
                auth_secret: SecretKeyOptional::Value(SecretKeyValue {
                    secret: "nordiblemailserver".into(),
                }),
                database: "nordiblemailserver".into(),
                use_tls: false,
    ...
    match typ {
        BlobStoreType::S3 => {
            ...
            BlobStore::S3(S3Store {
                ...
                bucket: "nordiblemailserver".into(),
            ...
            SearchStore::Meilisearch(MeilisearchStore {
                ...
                http_auth: HttpAuth::Bearer(HttpAuthBearer {
                    bearer_token: SecretKey::Value(SecretKeyValue {
                        secret: "nordiblemailserver-master-key".into(),
                    }),
                }),
                ..Default::default()
            })
