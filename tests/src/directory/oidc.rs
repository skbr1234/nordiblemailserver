...
pub async fn test() {
    ...
    let config = structs::OidcDirectory {
        ...
        issuer_url: "http://localhost:9080/realms/nordiblemailserver".to_string(),
        ...
        require_audience: Some("nordiblemailserver".to_string()),
    ...
    let token_openid_only = get_token_for_client(
        "nordiblemailserver-fallback",
        "nordiblemailserver-fallback-secret",
    ...
    assert_eq!(
        ...
        Some("http://localhost:9080/realms/nordiblemailserver/protocol/openid-connect/auth")
    );
}
...
async fn get_token_for_client(
    ...
    let response = client
        .post("http://localhost:9080/realms/nordiblemailserver/protocol/openid-connect/token")
        .form(&[
            ("grant_type", "password"),
            ("client_id", client_id),
